    ...
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
        ...
        echo '<nav class="navbar navbar-expand-lg navbar-light bg-primary p-3" style="border-radius: 5px ;">
            <div class="container-fluid">
               <a class="navbar-brand text-light" href="http://localhost:8080/tema8/chatgpt/admin/enrutador.php?accion=inicio">CHATGPT</a>
       ...
       foreach ($articulos as $articulo) {
        ...
        echo "<img width='240' height='275' class='card-img-top mt-2 rounded' src='".$articulo->getImagen()."'>";
        echo '<div class="card-body">';
        echo '<center><h5 class="card-title text-primary text-uppercase">' . $articulo->getTitulo() .'</h5></center>';
        echo '<p class="card-text h6"> '. $articulo->getTexto().'</p>';
        echo '<p class="card-text text-primary"> FECHA CREACIÓN:  '. $articulo->getFecha().'</p>';
        echo '</div>';
        echo "</div>";
    }
    echo '</div>';
